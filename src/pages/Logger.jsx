...
export default function Logger({ addToast }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!selectedItem || !amount || Number(amount) <= 0) {
      addToast({ type: 'warning', title: 'Invalid Input', message: 'Please select an activity and enter a valid amount.' })
      return
    }
  ...
  return (
    <div className="animate-fade-in-up">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
        ...
        <button className="btn btn-primary btn-lg" onClick={() => setShowForm(!showForm)} id="toggle-log-form">
      ...
      {showForm && (
        <div className="card animate-scale-in" style={{ marginBottom: 'var(--space-6)' }}>
          <h2 className="card-title" style={{ marginBottom: 'var(--space-4)' }}>Log New Activity</h2>
          <form onSubmit={handleSubmit}>
            ...
            {/* Notes */}
            <div className="form-group">
              <label className="form-label" htmlFor="activity-notes">Notes (optional)</label>
              <input
                id="activity-notes"
                type="text"
                className="form-input"
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
        <div className="tabs" style={{ marginBottom: 0, flex: 'unset' }}>
          ...
          <button className={`tab ${viewMode === 'heatmap' ? 'active' : ''}`} onClick={() => setViewMode('heatmap')}>📅 Heatmap</button>
        </div>
        <select
          className="form-select"
      ...
      {viewMode === 'heatmap' && (
        <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
          <h2 className="card-title" style={{ marginBottom: 'var(--space-4)' }}>Emissions Heatmap (12 weeks)</h2>
          <CalendarHeatmap data={heatmapData} weeks={12} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginTop: 'var(--space-3)', fontSize: 'var(--font-size-xs)', color: 'var(--text-tertiary)' }}>
            <span>Less</span>
            {['var(--bg-tertiary)', 'rgba(16,185,129,0.25)', 'rgba(16,185,129,0.45)', 'rgba(16,185,129,0.65)', 'rgba(16,185,129,0.9)'].map((c, i) => (
              ...
              return (
                <div key={dateStr} className="card">
                  <div className="card-header">
                    <div>
                      <h3 style={{ fontSize: 'var(--font-size-base)', fontWeight: 600 }}>{formatDate(dateStr)}</h3>
                      <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-tertiary)' }}>
                        {items.length} {items.length === 1 ? 'activity' : 'activities'}
                      </span>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 700, color: 'var(--color-primary)' }}>{dayTotal.toFixed(2)} kg</div>
                      <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-tertiary)' }}>CO₂</span>
                    </div>
                  </div>
                    ...
                    {items.map(activity => (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 500 }}>{activity.label}</div>
                        ...
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600 }}>{activity.emissions.toFixed(2)} kg</div>
                        </div>
                        <button
                          className="btn btn-sm"
                          style={{ color: 'var(--color-danger)', fontSize: '16px', padding: 'var(--space-1)' }}
      ...
      {/* Summary */}
      {activities.length > 0 && (
        <div style={{ marginTop: 'var(--space-6)', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 'var(--font-size-sm)' }}>
          Total: {activities.length} activities · {activities.reduce((s, a) => s + a.emissions, 0).toFixed(1)} kg CO₂
        </div>
      )}
    </div>
  )
